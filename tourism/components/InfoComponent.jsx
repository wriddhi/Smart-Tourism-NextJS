    ...
    if(place !== 'No query' && place !== ''){
      ...
      try{
        axios(url).then((response) => {
          ...
          context.updateResults(lat, lon, dt, temp_min, temp_max, temp, country, main)
          context.updateCenter(lat, lon)
          preventBan += 1;
        }).catch((error) => {
          console.log("Could not fetch weather from ", url)
          context.updateSearch('invalid')
          preventBan += 1;
        })
      }catch{
          console.log("Could not fucking fetch weather")
          preventBan+=1;
      }
    }
    ...
    return (
      <>
          <div className={infostyles.card}>
              ...
              <h3 className={infostyles.title}>{place}, {context.results.country}</h3>
